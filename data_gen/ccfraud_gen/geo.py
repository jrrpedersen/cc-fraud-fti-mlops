...
def far_country(rng: random.Random, exclude: set[str]) -> str:
    choices = [c.code for c in COUNTRIES if c.code not in exclude]
    return rng.choice(choices) if choices else rng.choice(COUNTRIES).code
